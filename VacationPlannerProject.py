...
def vacation_planner():
    ...
    # User input
    temp_preference = input("Do you want to go on a vacation to a warm or a cold place? ").capitalize()
    if temp_preference not in ["Warm", "Cold"]:
        print("Invalid input for temperature preference.")
        return
    
    #if temp_preference == "Warm":
        #vacation_purpose = input("What is the purpose of your vacation? (Beach, City trip, Cultural & Historic, Nature, Active): ").capitalize()
    #else:
        #vacation_purpose = input("What is the purpose of your vacation? (City trip, Cultural & Historic, Nature, Active, Skiing and Snowboarding): ").capitalize()

    #best_season = input("When are you going on vacation? (Summer, Autumn, Winter, Spring): ").capitalize()
    #duration = input("For how long are you going on vacation? (Weekend, Mid-week, Week, 10 days, 2 weeks, 3 weeks, Month): ").capitalize()
    #group_type = input("With whom are you going on vacation? (Alone, Best friend, My Partner, Group of girl friends, Group of Guy friends, Group of friends, Colleagues, My family, My parents, My siblings): ").capitalize()
